...
本目录包含 **HITSZ Connect CLI 1.3.6-hitsz.1** 的 Apple Silicon
(`darwin/arm64`) 构建，以及不含凭据的 Shadowrocket 配置。推荐 macOS 13+ 用户优先使用同仓库的
[HITSZ Connect.app](<../macos/HITSZ Connect.app>)；App 1.3.6 内置与本版本配套的 CLI，并用加密配置
代替命令行明文凭据。
...
| 路径 | 用途 |
| --- | --- |
| `hitsz-connect-darwin-arm64` | HITSZ Connect CLI 1.3.6-hitsz.1，Apple Silicon 可执行文件。 |
