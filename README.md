...
当前发布版本为 **HITSZ Connect 1.3.9**，内置 CLI 版本为 **1.3.9-hitsz.1**。
...
- 完整保留并拆分 aTrust 聚合 IPv4 帧，避免 Moonlight 等 UDP 视频流在入站路径丢包；
...
这些参数保留用于上游兼容和调试，不是 1.3.9 的推荐凭据存储方式：`-password` 可能进入 shell
